...
import emailData from "./Component/emailData";
...
const ShowCase = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, actions) => {
      setIsLoading(true);
      sendEmail(emailData(values, "matthew.eze@zikoramfb.com"));
      sendEmail(emailData(values, "peter.eze@zikoramfb.com"));

      sendEmail(emailData(values, "customer.service@zikoramfb.com")).then(
        (val) => {
          setIsLoading(false);
          if (val.status === 200) {
            actions.resetForm();
  ...
  return (
    <div>
        ...
        {!isLoading ? (
          <div className="flex flex-col gap-5 mb-4">
            ...
            <div className="flex justify-between">
              ...
              <ButtonRaw text="Send" onClick={formik.handleSubmit} />
            </div>
          </div>
        ) : (
      ...
      <Box
        className="h-full width-full pt-20 sm:pt-[2rem] flex flex-col items-center"
        sx={
          {
      ...
      >
        ...
        >
          <>
            <p className="text-[22px] sm:text-[18px] text-[#404040] font-[500]">
              Retail Loans
            </p>
            <p className="text-[16px] sm:text-[14px] text-[#5E5E5E] leading-[2rem]">
              We provide quick loans to our trusted users to ease the pressures
              they may be facing from their urgent financial needs.
            </p>
            <Button
          ...
          <>
            ...
            <p className="text-[16px] sm:text-[14px] text-[#5E5E5E] leading-[2rem]">
              Zikora will provide financial credit to worthy users and
              businesses looking to purchase tangible assets for personal and/or
              operational use.
            </p>
            <Button
              onClick={handleOpenModal}
        ...
        >
          <>
            <p className="text-[22px] sm:text-[18px] text-[white] font-[500]">
              Bridge Loan Finance
            </p>
            <p className="text-[16px] sm:text-[14px] text-[white] leading-[2rem]">
              We provide short term financing options to our credit worthy users
              to satisfy their urgent cash liquidity needs.
            </p>
            <Button
              onClick={handleOpenModal}
              sx={{
          ...
          <>
            ...
            <p className="text-[16px] sm:text-[14px] text-[#5E5E5E] leading-[2rem]">
              At Zikora, we understand the realities. Quite often, users might
              need funds prior to payday so we have created a package to get our
              users to payday hassle free.
            </p>
            <Button
              onClick={handleOpenModal}
              sx={{
        ...
        <CustomPlate rightPic="" leftPic={loanPic6} leftBg="#E8ECE3" rightBg="">
          <>
            <p className="text-[22px] sm:text-[18px] text-[#404040] font-[500]">
              Corporate Cash Advance
            </p>
            <p className="text-[16px] sm:text-[14px] text-[#5E5E5E] leading-[2rem]">
              We provide cash loans to help solve the urgent working capital
              needs of our user's businesses.
            </p>
            <Button
              onClick={handleOpenModal}
              sx={{
